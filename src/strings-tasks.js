...
function invertCase(str) {
  // Преобразуем строку, инвертируя регистр каждого символа
  return str
    .split('')
    .map((char) =>
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (c) => {
    ...
    // Если символ — заглавная буква (A-Z)
    if (charCode >= 65 && charCode <= 90) {
      return String.fromCharCode(((charCode - 65 + 13) % 26) + 65);
    }
    // Если символ — строчная буква (a-z)
    if (charCode >= 97 && charCode <= 122) {
      return String.fromCharCode(((charCode - 97 + 13) % 26) + 97);
    }

    // Если это не буква, возвращаем символ без изменений
    return c;
  });
}
...
function getCardId(str) {
  // Список всех значений карт (по порядку)
  const ranks = [
    '2',
    '3',
    '4',
  ...
  const rank = str.slice(0, -1); // Извлекаем ранг карты
  const suit = str.slice(-1); // Извлекаем масть карты

  // Проверяем, что карта существует в колоде
  ...
  // Индекс карты в колоде (по формуле: 13 * масть + ранг)
